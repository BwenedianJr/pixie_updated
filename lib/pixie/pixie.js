(function() {
  ...
  ScratchObj.prototype.redraw = function() {
    ...
    Object.defineProperty(ScratchObj.prototype, "width", {
    get: function () {
      return this.pixiSprite.width;
    },
    set: function (value) {
      this.pixiSprite.width = value;
    },
  });

  Object.defineProperty(ScratchObj.prototype, "height", {
    get: function () {
      return this.pixiSprite.height;
    },
    set: function (value) {
      this.pixiSprite.height = value;
    },
  });
  ...
  ScratchObj.prototype.updatePixi = function () {
    ...
    // Ensure the texture is properly loaded for the costume
    if (costume.baseLayerTexture) {
      // Check if the costume is a PNG and needs conversion
      if (costume.image && costume.image.endsWith(".png")) {
        this.convertPNGToSVG(costume); // Convert PNG to SVG
      }

      this.pixiSprite.setTexture(costume.baseLayerTexture);

      // Ensure the width and height are fetched from the Pixi texture object
      var texture = costume.baseLayerTexture;
      this.pixiSprite.width = texture.width;
      this.pixiSprite.height = texture.height;
    } else {
      this.pixiSprite.setTexture(this.pixiEmptyTexture);
      this.pixiSprite.width = 0;
      this.pixiSprite.height = 0;
    }
    ...
    this.pixiEffectsFilter.mosaic = Math.round(
      Math.abs(this.effects.mosaic) / 10 + 1
    );
    this.pixiEffectsFilter.whirlAngle = (-this.effects.whirl * Math.PI) / 180;
    this.pixiEffectsFilter.fisheye = Math.max(
      0.001,
      1 + this.effects.fisheye / 100
    );
    this.pixiEffectsFilter.pixelSize = this.effects.pixelate
      ? Math.min(
          Math.abs(this.effects.pixelate / 10) + 1,
          this.pixiSprite.width,
          this.pixiSprite.height
        )
      : 1;
    if (this.effects.pixelate || this.effects.whirl) {
      this.pixiEffectsFilter.uniforms.dimensions.value[0] =
        this.pixiSprite.width;
      this.pixiEffectsFilter.uniforms.dimensions.value[1] =
        this.pixiSprite.height;
    }

    // Set up the sprite positioning and scaling
    this.pixiSprite.scale.x = this.pixiSprite.scale.y = this.scale;
    this.pixiSprite.anchor.x = costume.cx / this.pixiSprite.width;
    this.pixiSprite.anchor.y = costume.cy / this.pixiSprite.height;
  ...
  Interpreter.prototype.addPrimitives = function(table) {
    ...
    table["say:"] = function (b) {
      var sprite = interp.activeThread.target;
      var bounds = sprite.pixiSprite.getBounds();
      var text = interp.arg(b, 0);

      if (!text.trim()) {
        // If text is empty or only spaces, remove the bubble if it exists
        var sprite = interp.activeThread?.target;
        if (sprite && sprite.customBubble) {
          document.body.removeChild(sprite.customBubble); // Remove the existing bubble
          sprite.customBubble = null; // Clear the reference
        }
        return; // Don't render the bubble if text is empty or only spaces
      }

      try {
        console.log("say: function is running");

        var sprite = interp.activeThread?.target; // Ensure sprite exists
        if (!sprite || !sprite.isSprite) {
          console.warn("say: function failed â sprite is missing or invalid.");
          return;
        }

        // Remove existing bubble if it exists
        if (sprite.customBubble) {
          // Remove the existing bubble (after rendering the new one)
          document.body.removeChild(sprite.customBubble);
          sprite.customBubble = null; // Clear the reference
        }

        // Create bubble canvas
        var canvas = document.createElement("canvas");
        canvas.className = "Visual-absolute bubble";
        canvas.style.position = "absolute";
        canvas.style.zIndex = "99999"; // Ensure it's always on top
        canvas.style.background = "transperent"; // TEMP: Make it visible for debugging
        document.body.appendChild(canvas); // Make sure it's actually added to the document
        ...
        var sx = 5,
          sy = 5,
          sb = 5;
        var sc = "rgba(0,0,0,0.5)";
        var r = 10;
        var bubbleMinWidth = 60;
        var maxWidth = 80; // Adjust bubble width
        var wrappedLines = wrapTextWithHyphenation(ctx, text, maxWidth);
        var lineHeight = 20;
        var numLines = wrappedLines.length;
        var bubbleHeight = Math.max(25, numLines * 18 + 12);
        ...
        textWidth += 20; // Add padding so text doesn't touch edges

        var w = textWidth + px + Math.max(px, ix);
        var i = Math.max(0, ix - px);
        var h = bubbleHeight + py * 2 + iy;

        var outlineThickness = 8; // Match stroke thickness

        // Expand canvas size to prevent cropping
        canvas.width = w + sx + sb * 2 + outlineThickness * 2;
        canvas.height = h + sy + sb * 2 + outlineThickness * 2;

        // Shift everything inward to keep it centered
        ctx.translate(sb + outlineThickness + 4, sb + outlineThickness);

        // Draw bubble shape
        ctx.beginPath();
        ctx.moveTo(i + px - ix, h + 8);
        ctx.lineTo(25 + i + px, h - iy);
        ctx.arc(i + r, h - iy - r, r, Math.PI / 2, Math.PI, false);
        ctx.arc(i + r, r, r, Math.PI, (Math.PI * 3) / 2, false);
        ctx.arc(w - r, r, r, (Math.PI * 3) / 2, 0, false);
        ctx.arc(w - r, h - iy - r, r, 0, Math.PI / 2, false);
        ctx.lineTo(10 + i + px - ix + iw - 5, h - iy);
        ctx.closePath();
        // Apply styles
        ctx.strokeStyle = "rgba(128, 128, 128, 1)"; // Gray outline
        ctx.lineWidth = outlineThickness; // Thicker outline
        ctx.stroke();
        ctx.fillStyle = "#fff";
        ...
        ctx.font = "16px 'Comic Sans MS', Arial, sans-serif";
        var textStartY = (h - iy) / 2 - ((numLines - 1) * lineHeight) / 2;
        wrappedLines.forEach((line, index) => {
          ctx.fillText(line, i + w / 2, textStartY + index * lineHeight);
        });
        // Store reference
        sprite.customBubble = canvas;

        // TEMP: Fixed position to see if it renders
        canvas.style.left = "100px";
        canvas.style.top = "100px";
        console.log("Bubble rendered at 100,100");

        // Positioning function
        function updateBubblePosition() {
          var textureWidth = sprite.width; // Get the width of the sprite's texture
          var textureHeight = sprite.height; // Get the height of the sprite's texture

          // Calculate bubble position (outside the sprite's texture)
          var x = sprite.x + textureWidth / 2 + 250; // Position the bubble slightly to the right of the sprite's texture
          var y = 0 - sprite.y - textureHeight / 2 - h + 250; // Position the bubble above the sprite

          // Adjust for the image size
          canvas.style.left = `${x}px`;
          canvas.style.top = `${y}px`;

          // Continuously update the bubble position
          requestAnimationFrame(updateBubblePosition);
        }

        updateBubblePosition(); // Start updating position
      } catch (error) {
        console.error("say: function error:", error);
      }
    };

    table["say:duration:elapsed:from:"] = function (b) {
      var sprite = interp.activeThread.target;
      var text = interp.arg(b, 0);
      var duration = interp.narg(b, 1); // Duration for how long to show the message
      ...
      try {
        console.log("say:duration:elapsed:from: function is running");
        ...
        // Remove existing bubble if it exists
        if (sprite.customBubble) {
          document.body.removeChild(sprite.customBubble); // Remove the old bubble
          sprite.customBubble = null; // Clear the reference
        }
        ...
        canvas.style.position = "absolute";
        canvas.style.zIndex = "99999"; // Ensure it's always on top
        canvas.style.background = "transparent"; // Make it visible for debugging
        ...
        // Store the reference to the bubble
        sprite.customBubble = canvas;
        ...
        // **Run the wait function inside the say block**
        if (interp.activeThread.tmp === null) {
          interp.startTimed(duration); // Start the timer for the given duration
        } else {
          // When the time is up, remove the bubble
          if (interp.stepTimed()) {
            if (sprite.customBubble) {
              document.body.removeChild(sprite.customBubble); // Remove the bubble
              sprite.customBubble = null; // Clear the reference
              console.log("Bubble removed after duration");
            }
          }
        }
      } catch (error) {
        console.error("say:duration:elapsed:from: function error:", error);
      }
    };

    table["think:"] = function (b) {
      var sprite = interp.activeThread.target;
      var bounds = sprite.pixiSprite.getBounds();
      var text = interp.arg(b, 0);
      ...
      try {
        console.log("think: function is running");

        var sprite = interp.activeThread?.target; // Ensure sprite exists
        if (!sprite || !sprite.isSprite) {
          console.warn(
            "think: function failed â sprite is missing or invalid."
          );
          return;
        }
        ...
        canvas.style.position = "absolute";
        canvas.style.zIndex = "99999"; // Ensure it's always on top
        canvas.style.background = "transparent"; // TEMP: Make it visible for debugging
        document.body.appendChild(canvas); // Make sure it's actually added to the document
        console.log("Canvas added to document");
        ...
        var outlineThickness = 8; // Match stroke thickness
        // Dots Parameters
        var dotX = i + px - ix + 10; // Start position for the first dot (left side of the bubble)
        var dotY = h + 10 + 5; // Y position for the dots, just below the bubble
        var baseDotSize = 6; // Initial size of the first dot (fixed height for consistency)
        var dotSpacing = 15; // Space between dots horizontally
        var dotOffsetY = 10; // Adjust this to make the dots lower if needed
        var upwardOffset = 8; // The amount the dots move up for each iteration
        var outlineThickness = 4; // Ensure the outline thickness is the same for all dots

        // Expand canvas size to prevent cropping
        canvas.width =
          w + sx + sb * 2 + dotSpacing * 2 + 3 * (baseDotSize + i * 2);

        canvas.height = h + sy + sb * 2 + dotY + dotOffsetY + upwardOffset * 3;
        ...
        ctx.arc(w - r, r, r, (Math.PI * 3) / 2, 0, false);
        ctx.arc(w - r, h - iy - r, r, 0, Math.PI / 2, false);
        ctx.closePath(); // Finish the path for the bubble

        // Set the fill and stroke for the bubble
        ctx.fillStyle = "#fff"; // White fill
        ctx.strokeStyle = "rgba(128, 128, 128, 1)"; // Gray outline
        ctx.lineWidth = 4; // Line width for the outline
        ctx.fill(); // Fill the bubble
        ctx.stroke(); // Draw the outline

        // Loop to create three dots, gradually increasing their width (but maintaining constant height)
        for (let i = 0; i < 3; i++) {
          // Draw the sideways oval (ellipse) for each dot
          ctx.beginPath();
          // Create ovals with increasing width, fixed height
          ctx.ellipse(
            dotX,
            dotY + dotOffsetY,
            baseDotSize + i * 2,
          ...
          // Oval shape
          ctx.fillStyle = "#fff"; // White fill for the dots
          ctx.strokeStyle = "rgba(128, 128, 128, 1)"; // Gray outline for the dots
          ctx.lineWidth = outlineThickness; // Set a consistent outline thickness for all dots
          ctx.fill();
          ctx.stroke();
          outlineThickness += i * 1.5;
          // Move the x position for the next dot
          dotX += dotSpacing;
          dotSpacing -= 5;
          upwardOffset += 3;
          // Move the y position upwards gradually for the next dot
          dotY -= upwardOffset; // Decrease Y to make dots go upwards
        }
        ...
        console.error("think: function error:", error);
      }
    };

    table["think:duration:elapsed:from:"] = function (b) {
      var sprite = interp.activeThread.target;
      var text = interp.arg(b, 0);
      var duration = interp.narg(b, 1); // Duration for how long to show the message
    ...
    function wrapTextWithHyphenation(ctx, text, maxWidth) {
    ...
    for (let i = 0; i < words.length; i++) {
        ...
        if (testWidth > maxWidth) {
            if (ctx.measureText(words[i]).width > maxWidth) {
                // Word itself is too long, apply hyphenation
                let hyphenated = hyphenateWord(ctx, words[i], maxWidth);
                lines.push(currentLine); // Push current line before hyphenating word
                lines = lines.concat(hyphenated);
                currentLine = "";
            } else {
    ...
    if (currentLine) lines.push(currentLine); // Add last line
    return lines;
}
...
function hyphenateWord(ctx, word, maxWidth) {
    ...
    for (let i = 0; i < word.length; i++) {
        ...
        let testWidth = ctx.measureText(testPart).width;

        if (testWidth > maxWidth) {
            // Add hyphen and start a new part
    ...
    table["doAsk"] = function (b) {
      ...
      try {
        console.log("doAsk function is running");

        var sprite = interp.activeThread?.target; // Ensure sprite exists
        if (!sprite || !sprite.isSprite) {
          console.warn("doAsk function failed â sprite is missing or invalid.");
          return;
        }
        ...
        // Apply styles
        ctx.strokeStyle = "#2ca5e2"; // Scratch 2.0 blue outline
        ctx.lineWidth = outlineThickness; // Thicker outline
        ctx.stroke();
        ctx.fillStyle = "#fff";
        ctx.shadowColor = "transparent";
        ...
        function updateBubblePosition() {
          ...
          // Calculate bubble position (outside the sprite's texture)
          var x = sprite.x + textureWidth / 2 + 250; // Position the bubble slightly to the right of the sprite's texture
          var y = 0 - sprite.y - textureHeight / 2 - h + 280; // Position the bubble above the sprite
        ...
        console.error("doAsk function error:", error);
      }

      // Create input container
      var inputContainer = document.createElement("div");
      inputContainer.style.position = "absolute";
      inputContainer.style.left = "10px"; // Adjust as needed
      inputContainer.style.top = "365px"; // Adjust as needed
      inputContainer.style.zIndex = "99999";
      inputContainer.style.display = "flex";
      inputContainer.style.alignItems = "center";
      inputContainer.style.padding = "5px";
      inputContainer.style.border = "6px solid #2ca5e2";
      inputContainer.style.borderRadius = "8px";
      inputContainer.style.backgroundColor = "#fff";
      // Removed shadow to match the flat look you want
      inputContainer.style.boxShadow = "none";

      // Create text input with 3D indent
      var inputBox = document.createElement("input");
      inputBox.type = "text";
      inputBox.style.width = "400px"; // Adjust as needed
      inputBox.style.height = "30px";
      inputBox.style.border = "none";
      inputBox.style.outline = "none";
      inputBox.style.fontFamily = "'Comic Sans MS', Arial, sans-serif";
      inputBox.style.fontSize = "14px";
      inputBox.style.backgroundColor = "#fff"; // White background for input box
      inputBox.style.padding = "5px";
      // Adding a 3D indent effect to the input box
      inputBox.style.boxShadow =
        "inset 2px 5px 8px rgba(0, 0, 0, 0.2), inset -3px -3px 8px rgba(255, 255, 255, 0.6)";

      // Create check mark image
      var checkMarkImage = new Image();
      checkMarkImage.src =
        "https://raw.githubusercontent.com/BwenedianJr/png/main/CheckMark.PNG"; // Your correct image URL
      checkMarkImage.style.width = "30px"; // Adjust size as needed
      checkMarkImage.style.height = "30px"; // Adjust size as needed
      checkMarkImage.style.marginLeft = "10px"; // Adjust margin to position the check mark next to the input box

      // Append elements
      inputContainer.appendChild(inputBox);
      inputContainer.appendChild(checkMarkImage); // Append the checkmark image here
      document.body.appendChild(inputContainer);

      // Auto-focus input
      inputBox.focus();

      // Handle user input when pressing Enter or clicking check button
      function submitInput() {
        var userInput = inputBox.value;
        console.log("User input:", userInput);
        inputContainer.style.display = "none"; // Hide input box after submission
        document.body.removeChild(sprite.customBubble);
        table["answer"](userInput);
      }
      ...
      checkMarkImage.addEventListener("click", submitInput);
    };
